...
"""
RFT Compressibility Probe — Classical Quantum State Simulation
==============================================================

Implements a "Compressed Simulation" loop using the Φ-RFT stack to probe
how compressible structured quantum state vectors are in the RFT domain,
and to demonstrate a sparse simulation loop.

IMPORTANT SCOPE NOTE
--------------------
This is a **classical** simulation.  It stores the full 2^k complex-amplitude
state vector (exponential classical cost) and applies standard quantum gates
as NumPy matrix multiplications.  There is no quantum hardware, no physical
qubit, and no quantum speedup.

The term "quantum" refers to: the mathematical structure of the state vector
(complex amplitudes summing to 1 in L2 norm) and the unitary gate operations
(Hadamard, CNOT, etc.).  All computation is classical.

The key research question addressed here is: can φ-RFT transform the
amplitude vector into a *sparser* representation than DFT for φ-periodic
quantum states?  The answer (empirically observed) is yes for structured
initial conditions, with sparsity levels consistent with Theorem 8.

Usage
-----
    python3 algorithms/rft/compression/rft_quantum_sim.py
"""
