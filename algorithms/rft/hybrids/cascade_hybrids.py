...
Licensed under LicenseRef-quantoniumos-Claims-NC
"""
...
# Prefer native CASMâ†’C++ optimized RFT; fall back to Python
try:
...
class H3HierarchicalCascade:
    ...
    Properties:
        ...
        2. Zero coherence: Î· = 0 (no inter-basis competition between DCT/RFT domains)
    ...
    Note on Energy Preservation:
        ...
        For EXACT energy preservation ||x||Â² = ||x_struct||Â² + ||x_texture||Â² + ||x_residual||Â²,
    ...
    def __init__(self, kernel_size_ratio: float = 1/32):
        ...
        # Instantiate native RFT if available (uses CASMâ†’C++ pipeline)
        self.rft = None
        if _RFT_NATIVE:
            try:
...
class FH5EntropyGuided(H3HierarchicalCascade):
    ...
    Mechanism:
        Local entropy H_local[i] = -Î£ p(x) logâ‚‚ p(x) in window around i
        High entropy â†’ RFT (texture domain)
        Low entropy â†’ DCT (structure domain)
    ...
    def _adaptive_decompose(self, signal: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        High entropy regions â†’ texture (RFT)
        Low entropy regions â†’ structure (DCT)
