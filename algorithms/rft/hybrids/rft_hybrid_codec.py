...
from algorithms.rft.core.resonant_fourier_transform import (
    rft_forward_square as _rft_forward_dense,
    rft_inverse_square as _rft_inverse_dense,
    _MATRIX_FREE_THRESHOLD,
    _operator_adjoint,
    _operator_synthesis,
    rft_forward_iht,
)
...
# Re-export the original names for callers that import them from here
rft_forward = _rft_forward_dense
rft_inverse = _rft_inverse_dense
...
def rft_forward_real(vec: np.ndarray) -> np.ndarray:
    """Forward RFT -- matrix-free for large N to avoid O(N^2) memory."""
    n = len(vec)
    if n > _MATRIX_FREE_THRESHOLD:
        # Use IHT with generous keep_k for quality, operator-only
        keep_k = max(16, n // 8)
        return rft_forward_iht(vec.real.astype(np.float64), keep_k=keep_k)
    return _rft_forward_dense(vec)


def rft_inverse_real(coeffs: np.ndarray) -> np.ndarray:
    """Inverse RFT -- matrix-free for large N to avoid O(N^2) memory."""
    n = len(coeffs)
    if n > _MATRIX_FREE_THRESHOLD:
        return _operator_synthesis(coeffs).real
    rec = _rft_inverse_dense(coeffs)
    return rec.real
...
def encode_tensor_hybrid(
    ...
    precomputed_coeffs: Optional[np.ndarray] = None,
    mode: str = "crypto",
    ...
    n = flat.size
    coeffs = precomputed_coeffs if precomputed_coeffs is not None else rft_forward_real(flat)
    amps = np.abs(coeffs)
    phases = np.angle(coeffs)
    log_amps = np.log(np.maximum(amps, 1e-12))

    # --- Rate control: compress mode uses top-k thresholding ---
    if mode == "compress" and target_bpv is not None:
        bits_per_coeff = math.log2(max(n, 2)) + 16  # index + complex value
        k = max(1, int(target_bpv * n / bits_per_coeff))
        k = min(k, n)
        if k < n:
            topk_idx = np.argsort(amps)[-k:]
            keep_mask = np.zeros(n, dtype=bool)
            keep_mask[topk_idx] = True
        else:
            keep_mask = np.ones(n, dtype=bool)
        kept_indices = np.nonzero(keep_mask)[0]
    # --- Crypto / legacy: threshold by absolute amplitude ---
    elif prune_threshold > 0:
        keep_mask = amps >= prune_threshold
        kept_indices = np.nonzero(keep_mask)[0]
    else:
        keep_mask = np.ones_like(amps, dtype=bool)
        kept_indices = np.nonzero(keep_mask)[0]
    ...
    container = {
        ...
        "codec": {
            "mode": mode if mode == "compress" else "hybrid",
            "quant_amp_bits": int(quant_amp_bits),
            "quant_phase_bits": int(quant_phase_bits),
            "prune_threshold": float(prune_threshold),
            **({"target_bpv": float(target_bpv)} if mode == "compress" and target_bpv is not None else {}),
        },
        "bands": band_meta,
...
def decode_tensor_hybrid(container: Dict, predictor: Optional[TinyResidualPredictor] = None,
                         inverse_func=None) -> np.ndarray:
    ...
    if inverse_func is not None:
        flat_rec = np.asarray(inverse_func(coeffs)).real
    else:
...
class RFTHybridCodec:
    ...
    def encode(self, tensor: np.ndarray, precomputed_coeffs=None,
               codec_mode: str = "crypto", target_bpv: Optional[float] = None) -> Dict:
        """Encode tensor using selected hybrid mode.

        Args:
            codec_mode: "crypto" for full-fidelity round-trip, "compress" for
                        rate-controlled top-k thresholding.
            target_bpv: Target bits per value (only used when codec_mode="compress").
        """
        if self.mode == 'legacy':
            return encode_tensor_hybrid(
                tensor, precomputed_coeffs=precomputed_coeffs,
                mode=codec_mode, target_bpv=target_bpv,
                **self.encode_kwargs,
            ).container
        else:
    ...
    def decode(self, container: Dict, predictor: Optional[TinyResidualPredictor] = None,
               inverse_func=None) -> np.ndarray:
        """Decode tensor using original encoding mode"""
        if container.get("type") == "rft_hybrid_tensor":
            # Legacy format
            return decode_tensor_hybrid(container, predictor, inverse_func=inverse_func)
