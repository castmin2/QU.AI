...
Theorem 4.1 (Hybrid Basis Decomposition):
    ...
    satisfying:
        ...
        3. ||x||Â² = ||x_struct||Â² + ||x_texture||Â² + ||x_residual||Â² (exact Parseval identity)
...
def theoretic_hybrid_decomposition(
    ...
    This uses orthonormal basis projections to guarantee exact Parseval identity:
        ||x||Â² = ||x_struct||Â² + ||x_texture||Â² + ||x_residual||Â²
...
def verify_theorem_4_1():
    ...
    for name, signal in test_cases.items():
        ...
        print(f"  Energy error:        {result.energy_error:.2e}  {'âœ“' if result.energy_error < 1e-12 else 'âœ— FAILED'}")
        ...
        print(f"  Reconstruction error: {reconstruction_error:.2e}  {'âœ“' if reconstruction_error < 1e-12 else 'âœ— FAILED'}")
