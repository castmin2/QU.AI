...
Performance target: <20ms for 256Ã—256 images (10Ã— speedup)
...
def anscombe_transform(image: np.ndarray) -> np.ndarray:
    ...
    Transforms Poisson-distributed data to approximately Gaussian with Ïƒâ‰ˆ1.
    f(x) = 2 * sqrt(x + 3/8)
    """
...
def inverse_anscombe(transformed: np.ndarray) -> np.ndarray:
    ...
    Uses the algebraic inverse: x = (y/2)Â² - 3/8
...
def fast_rft_2d(image: np.ndarray) -> np.ndarray:
    ...
    ~10Ã— faster than row-by-row Python loops.
    """
...
def rft_wavelet_denoise_v2(noisy_image: np.ndarray,
    ...
    if noise_model == "poisson":
        ...
        # After Anscombe, noise is approximately Gaussian with Ïƒâ‰ˆ1
    ...
    for level in range(levels - 1, -1, -1):
        ...
        if noise_model == "rician":
            ...
            # After Anscombe, treat as Gaussian with Ïƒâ‰ˆ1
            threshold_mult = 1.0
            level_noise_var = 1.0  # Anscombe stabilizes to Ïƒâ‰ˆ1
        else:
            threshold_mult = 1.0
...
def rft_wavelet_denoise_v2_adaptive(noisy_image: np.ndarray,
    ...
    # Poisson: variance â‰ˆ mean, low skewness
