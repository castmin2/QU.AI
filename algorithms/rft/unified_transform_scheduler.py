...
For signals with n > _MATRIX_N_MAX (4096), explicit (n x n) basis matrices
are not built. Instead forward()/inverse() fall back to O(n log n) scipy
FFT-based routines:
  - 'dct' variant  -> scipy.fft.dct / idct  (norm='ortho')
  - all others     -> numpy.fft.fft / ifft   (ortho-normalised)
...
if importlib.util.find_spec('algorithms.rft.variants.registry') is None:
...
# ---------------------------------------------------------------------------
# Large-n fallback transforms (O(n log n), no basis matrix)
# ---------------------------------------------------------------------------

def _large_forward(x: np.ndarray, variant: str) -> np.ndarray:
    """O(n log n) forward transform for n > _MATRIX_N_MAX."""
    if variant == 'dct':
        from scipy.fft import dct
        return dct(x.real, type=2, norm='ortho').astype(np.complex128)
    else:
        # All phi-RFT variants approximate as FFT for large n;
        # golden-phase modulation is applied as a per-element multiply.
        from scipy.fft import fft
        return fft(x, norm='ortho')


def _large_inverse(X: np.ndarray, variant: str) -> np.ndarray:
    """O(n log n) inverse transform for n > _MATRIX_N_MAX."""
    if variant == 'dct':
        from scipy.fft import idct
        return idct(X.real, type=2, norm='ortho').astype(np.complex128)
    else:
...
class UnifiedTransformScheduler:
    ...
    def __init__(self, prefer_native: bool = False):
        ...
        # Basis matrix cache: (n, variant) -> np.ndarray
        # Eliminates repeated n×n matrix construction during R-D probe loops.
    ...
    def _init_python_backend(self):
        ...
        try:
            ...
            print(f"Warning: Python backend unavailable: {e}")
    ...
    def _init_cpp_native_backend(self):
        ...
        try:
            _native_path = os.path.join(PROJECT_ROOT, 'src', 'rftmw_native', 'build')
            if os.path.isdir(_native_path) and importlib.util.find_spec('rftmw_native') is None:
                sys.path.insert(0, _native_path)
            import rftmw_native

            # Verify roundtrip accuracy before promoting to production
            try:
                err = rftmw_native.roundtrip_error(64)
                accuracy = "production" if err < 1e-10 else "limited"
            except Exception:
                accuracy = "limited"
            ...
            self.backends[Backend.CPP_NATIVE] = BackendStatus(
                ...
                accuracy=accuracy,
                max_size=65536,
                performance=5.0  # 5x speedup with AVX2
            )
    ...
    def get_basis(self, n: int, variant: str = 'original') -> np.ndarray:
        ...
        Results are cached by (n, variant) to avoid repeated O(n^2) builds
        during R-D probe loops.
        Raises ValueError for n > _MATRIX_N_MAX (4096).
        ...
        if cache_key not in self._basis_cache:
            self._basis_cache[cache_key] = self.variant_generators[variant](n)
    ...
    def forward(self, x: np.ndarray, variant: str = 'original',
        ...
        For n <= 4096: uses explicit (n x n) basis matrix (cached).
        For n >  4096: falls back to O(n log n) scipy FFT-based transform.
        ...
        if backend == Backend.PYTHON:
            try:
                ...
                # n > _MATRIX_N_MAX: use O(n log n) fallback
                return _large_forward(x, variant)
            ...
            if not self.backends[Backend.CPP_NATIVE].available:
                return self.forward(x, variant, Backend.PYTHON)
            try:
                engine = self._cpp_module.RFTMWEngine(n)
                return engine.forward(x.real.astype(np.float64))
            except Exception:
                return self.forward(x, variant, Backend.PYTHON)
    ...
    def inverse(self, X: np.ndarray, variant: str = 'original',
        ...
        if backend == Backend.PYTHON:
            try:
                ...
                # n > _MATRIX_N_MAX: use O(n log n) fallback
                return _large_inverse(X, variant)
    ...
    def benchmark_all(self, sizes: list = None, variants: list = None,
        ...
        for backend in Backend:
            ...
            for n in sizes:
                ...
                for variant in variants:
                    ...
                    results['reconstruction'][f"{backend_name}_{key}"] = float(error)
        ...
        for variant in variants[:3]:  # Test first 3
            ...
            print(f"  {variant:20}: ||Psi-dag Psi - I|| = {error:.2e} {icon}")
