...
"""
Quantum Search (Classical Simulation) — quantoniumos
=====================================================

Classical full-state-vector simulation of Grover's search algorithm
applied to GeometricContainers.

IMPORTANT SCOPE NOTE
--------------------
This is a **classical simulation**.  It stores and manipulates the full
2^k complex-amplitude state vector, so it has *exponential* classical cost.
There is no quantum hardware, no physical qubit, and no quantum speedup.
On a classical computer Grover's algorithm provides no advantage over a
linear scan of the container list.

The implementation follows Nielsen & Chuang (2000), Chapter 6.  The Oracle
phase-flips the amplitude of the target basis state; the Grover diffusion
operator then amplifies it.  After O(√N) iterations the measurement
collapses to the target with high probability — modelled here as returning
the container at the peak-amplitude index.

Why this exists
---------------
The GeometricContainer objects encode RFT-domain hash data (Claim 3).
This module demonstrates that standard quantum search circuits are
compatible with φ-basis unitary gates without modification; it does NOT
claim quantum speedup on classical hardware.
"""
...
class QuantumSearch:
    """Classical simulation of Grover's search over GeometricContainers.

    State representation: 2^k complex amplitudes (exponential classical cost).
    Suitable for small container lists (≤ 2^20 entries) only.
    """
