...
# Golden angle (phyllotaxis): 2Ï€/Ï†Â² â‰ˆ 137.5Â° (complement 2Ï€/Ï† â‰ˆ 222.5Â°)
...
def generate_rft_polar_golden(n: int) -> np.ndarray:
    ...
    R[k] = cos(Î¸_k) * r_k where r_k = Ï†^(k/n) and Î¸_k = 2Ï€k/n
    """
    k = np.arange(n)
    theta = 2 * np.pi * k / n
    radius = PHI ** (k / n)  # Golden ratio radial scaling
    
    # Polar to Cartesian: x = r*cos(Î¸)
...
def generate_rft_spiral_golden(n: int) -> np.ndarray:
    ...
    r(Î¸) = a * e^(bÎ¸) where b = ln(Ï†)/(Ï€/2) for golden spiral
    ...
    # Golden spiral: b chosen so r doubles each quarter turn by Ï†
...
def generate_rft_exp_decay_golden(n: int) -> np.ndarray:
    ...
    R[k] = e^(-k/Ï„) * cos(2Ï€ Ï† k / n) where Ï„ = n/Ï†
...
def generate_rft_mobius(n: int) -> np.ndarray:
    """
    RFT-Mobius: MÃ¶bius transformation resonance.
    
    MÃ¶bius maps preserve circles and angles - key to conformal mappings.
    Uses z -> (az+b)/(cz+d) structure with golden ratio coefficients.
    """
    k = np.arange(n)
    z = np.exp(2j * np.pi * k / n)  # Unit circle
    
    # MÃ¶bius transformation with golden coefficients
    a, b = PHI, 1.0
    c, d = 1.0, PHI
...
def generate_rft_hopf_fibration(n: int) -> np.ndarray:
    ...
    SÂ³ â†’ SÂ² fibration with golden ratio fiber twist.
    ...
    # SÂ³ to SÂ² projection (stereographic)
...
PATENT_VARIANTS = {
    ...
    'rft_mobius': {
        ...
        'description': 'MÃ¶bius transformation with golden coefficients',
    ...
    'rft_hopf_fibration': {
        ...
        'description': 'SÂ³â†’SÂ² Hopf fibration resonance',
        'claim': 'Claim 3: manifold mappings',
    },
...
# =============================================================================
# PATENT CLAIM 4: RoPE-Complementary Phi-Basis
# =============================================================================

@lru_cache(maxsize=32)
def generate_rft_rope_complement(n: int, rope_base: float = 10000.0) -> np.ndarray:
    """
    RFT-RoPE-Complement: Phi-basis designed to complement Rotary Positional
    Embeddings instead of interfering with them.

    Patent Claim 4: Hybrid Mode Integration — architecture-aware variant
    routing that adapts the phi-basis to the target model's positional
    encoding geometry.

    Construction:
    1. Compute RoPE's rotation frequencies: theta_i = rope_base^(-2i/d)
       for each dimension pair (2i, 2i+1).
    2. Build an autocorrelation R[k] whose spectral energy is concentrated
       in the GAPS between RoPE's frequencies, using the golden ratio to
       place tones at maximally irrational offsets from each theta_i.
    3. Construct the Toeplitz resonance operator and extract its eigenbasis.

    This is non-circular: the variant uses RoPE's published frequency
    schedule (Vaswani 2017, Su et al. 2021) as an input parameter, not
    any model weights or training data. The basis is determined entirely
    by the dimension size and the rope_base constant.
    """
    k = np.arange(n)
    t = k / n

    # Step 1: Compute RoPE's rotation frequencies for dimension pairs.
    # For head_dim=d, RoPE uses d/2 distinct frequencies.
    n_pairs = n // 2
    rope_freqs = rope_base ** (-2.0 * np.arange(n_pairs) / n)

    # Step 2: Build complementary autocorrelation.
    # Place phi-shifted tones in the spectral gaps between RoPE frequencies.
    # The golden ratio offset ensures maximal irrationality with respect to
    # each RoPE frequency (Three-Distance Theorem property).
    r = np.zeros(n, dtype=np.float64)

    for i, theta_i in enumerate(rope_freqs):
        # Golden-ratio offset from each RoPE frequency
        # phi_freq sits at the maximally irrational point between theta_i
        # and its neighbor, avoiding destructive interference
        phi_offset = theta_i * PHI / (PHI + 1)  # golden section of theta_i
        anti_offset = theta_i / PHI              # reciprocal golden offset

        # Weight by inverse frequency index — low-frequency pairs carry more
        # information in attention (they encode coarse position)
        weight = 1.0 / (1.0 + 0.5 * i)

        # Complementary tones: placed at golden offsets from RoPE freqs
        r += weight * np.cos(2 * np.pi * phi_offset * t)
        r += weight * 0.5 * np.cos(2 * np.pi * anti_offset * t)

    # Step 3: Add the phase-coherent coupling term from pat_rft_phase_coherent.
    # This preserves the coupled-oscillator structure that already showed
    # 47% gap reduction on TinyLlama.
    omega1 = 2 * np.pi * 5
    omega2 = 2 * np.pi * 5 * PHI
    coupling = 0.2  # slightly lower coupling to avoid overmodulation
    phase_coherent = np.cos(omega1 * t) + np.cos(omega2 * t + coupling * np.sin(omega1 * t))
    r += 0.3 * phase_coherent  # blend: 70% complementary, 30% phase-coherent

    # Normalize
    r = r / (np.max(np.abs(r)) + 1e-10)

    # Step 4: Build operator and extract eigenbasis
    K = _build_resonance_operator(r, decay_rate=0.008)
    return _eigenbasis(K)


@lru_cache(maxsize=32)
def generate_rft_rope_pure(n: int, rope_base: float = 10000.0) -> np.ndarray:
    """
    RFT-RoPE-Pure: Pure complementary basis without phase-coherent blending.

    Same as RoPE-Complement but without the phase_coherent coupling term.
    Used as an ablation to isolate the contribution of the complementary
    frequency placement vs the coupled-oscillator structure.
    """
    k = np.arange(n)
    t = k / n
    ...
    r = np.zeros(n, dtype=np.float64)
    for i, theta_i in enumerate(rope_freqs):
        phi_offset = theta_i * PHI / (PHI + 1)
        anti_offset = theta_i / PHI
        weight = 1.0 / (1.0 + 0.5 * i)
        r += weight * np.cos(2 * np.pi * phi_offset * t)
        r += weight * 0.5 * np.cos(2 * np.pi * anti_offset * t)
...
if __name__ == "__main__":
    ...
    for name, info in PATENT_VARIANTS.items():
        try:
            ...
            print(f"âœ— {info['name']:<25} | ERROR: {e}")
