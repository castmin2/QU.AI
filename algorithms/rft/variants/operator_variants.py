...
def generate_rft_fibonacci(n: int) -> np.ndarray:
    ...
    R[k] = Î£ cos(2Ï€ F_i k / n) for Fibonacci numbers F_i
...
def generate_rft_harmonic(n: int, num_harmonics: int = 5) -> np.ndarray:
    ...
    R[k] = Î£ (1/i) cos(2Ï€ i fâ‚€ k / n) for i = 1, 2, ..., num_harmonics
...
def generate_rft_geometric(n: int) -> np.ndarray:
    ...
    R[k] = Î£ cos(2Ï€ Ï†^i k / n) for i = 0, 1, 2, ...
...
def generate_rft_beating(n: int) -> np.ndarray:
    ...
    R[k] = cos(2Ï€ fâ‚ k/n) + cos(2Ï€ fâ‚‚ k/n) where fâ‚‚ = Ï†Â·fâ‚
...
def generate_rft_phyllotaxis(n: int) -> np.ndarray:
    ...
    Based on the phyllotaxis divergence angle 2Ï€/Ï†Â² â‰ˆ 137.5Â°
    (complement 2Ï€/Ï† â‰ˆ 222.5Â°, depending on rotation direction).
...
OPERATOR_VARIANTS = {
    ...
    'rft_phyllotaxis': {
        ...
        'description': 'Golden angle (137.5Â°) structure',
