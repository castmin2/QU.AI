...
"""
Python bindings for the bare-metal Unitary RFT (Resonant Fourier Transform).
============================================================================

Provides ctypes-based Python bindings to the C/ASM implementation of the
Resonant Fourier Transform (RFT) — a unitary transform that uses golden-ratio
(φ) frequency spacing instead of integer harmonics.

The canonical RFT is defined as:

    Φ_{n,k} = (1/√N) exp(j 2π frac((k+1)φ) n)
    U = Φ (ΦᴴΦ)^{-1/2}       (Gram/Löwdin normalization → exact unitary)

This binding layer exposes the AVX2/AES-NI accelerated C++ kernels from
``src/rftmw_native/`` through ctypes.  The engine falls back to the pure
Python implementation in ``algorithms.rft.core.canonical_true_rft`` when the
native shared library is unavailable.

License: LicenseRef-quantoniumos-Claims-NC (research/education only)
Patent:  USPTO 19/169,399, Claims 1 & 4

IMPORTANT
---------
Keep :class:`RFTEngine` field layout exactly in sync with ``rft_engine_t`` in
...
misalign the struct, leading to heap corruption (historically observed as a
GC crash when topological fields were omitted).
"""
...
# RFT Variants (mirrors rft_kernel.h)
...
RFT_VARIANT_HYPERBOLIC = 6
RFT_VARIANT_DCT = 7
RFT_VARIANT_HYBRID_DCT = 8
RFT_VARIANT_CASCADE = 9
RFT_VARIANT_ADAPTIVE_SPLIT = 10
RFT_VARIANT_ENTROPY_GUIDED = 11
RFT_VARIANT_DICTIONARY = 12
RFT_VARIANT_CANONICAL = 13
RFT_VARIANT_BINARY_WAVE = 14
RFT_VARIANT_CRYPTO_SIS = 15

# Backward compatibility aliases
RFT_VARIANT_HYBRID = RFT_VARIANT_PHI_CHAOTIC
RFT_VARIANT_ADAPTIVE = RFT_VARIANT_ADAPTIVE_SPLIT
